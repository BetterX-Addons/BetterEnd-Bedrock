import { ItemStack, system, BlockPermutation } from "@minecraft/server";
            ...
            if (isSlab.has(selectedItem.typeId)) {
                ...
                if ((isBottomUp || isTopDown) && selectedItem.typeId.replace("", "") === block.typeId) {
                    ...
                    if (!block.permutation.getState("betterend:double")) {
                        ...
                        placeSlab(player, selectedItem, adjacentBlockPos, face, equipment, block, true);
                    }
                }
                    ...
                    placeSlab(player, selectedItem, adjacentBlockPos, face, equipment, block, false);
                }
            }
            else {
...
export function placeSlab(player, selectedItem, adjacentBlockPos, face, equipment, targetBlock, isDoubleSlab) {
    let verticalHalf;
    ...
    if (raycastHit && isDoubleSlab) {
        ...
        if (face === "Up") {
            ...
            if (isSlab.has(targetBlock.typeId)) {
                verticalHalf = targetBlock.permutation.getState("minecraft:vertical_half");
            }
            else {
...
function generateStructureName(blockTypeId, verticalHalf) {
    ...
    const structureName = verticalHalf === "bottom"
